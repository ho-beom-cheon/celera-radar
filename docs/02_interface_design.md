...
```json
{
  ...
  "trend": {
    ...
    "warnings": ["데이터랩 ratio는 검색 클릭 추이 기반이며 실제 판매량이 아닙니다."]
  },
  "score": null
}
```
...
`trend`는 같은 `snapshotDate`로 저장된 최신 일별 스냅샷 묶음만 사용한다. 서로 다른 수집 기준일의 point를 섞지 않는다.
키워드 단위 `score`는 후보별 가격·공급가·위험 정보가 없어 현재 `null`이며, 실제 종합점수는 Candidate API에서 제공한다.
...
### POST /candidates/recalculate

인증 사용자의 제외되지 않은 후보를 최신 저장 스냅샷으로 재계산한다. 키워드가 연결되지 않은 후보는 건너뛰며 외부 API를 직접 호출하지 않는다.

Response:

```json
{
  "recalculatedCount": 12,
  "skippedCount": 2
}
```

재계산 시 `trendScore`는 키워드별 최신 Shopping Insight 스냅샷 묶음에서 계산하고, 쇼핑 경쟁 지표는 최신 쇼핑 검색 스냅샷을 사용한다.
