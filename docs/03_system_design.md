...
외부 연동은 business service가 provider port에만 의존하고 configured router가 mode별 adapter를 선택한다.

```text
ShoppingSearchSnapshotService -> ShoppingSearchProvider -> LEGACY | HUB | DISABLED
TrendSnapshotService          -> ShoppingInsightProvider -> LEGACY | HUB | DISABLED
```

| mode | 활성 조건 | 동작 |
|---|---|---|
| `LEGACY` | 기존 client id/secret 모두 존재 | 개발자센터 adapter 사용 |
...
3. 업로드 request/file은 plan과 무관하게 10 MB hard cap을 적용한다.
4. `.csv`, `.xlsx` 확장자, MIME, magic signature를 교차 확인한 뒤에만 파일을 수락한다.
5. 승인된 원본은 UUID key로 webroot 외부 private quarantine에 저장하고 저장 경로를 API로 노출하지 않는다.
6. 검증·파싱·DB transaction 실패 시 quarantine 파일을 함께 제거한다.
7. XLSX는 ZIP 구조·압축 비율·압축 해제 크기와 sheet/row/column/cell 제한을 통과한 뒤 SAX/event parser로 첫 sheet만 읽는다.
8. 원본은 기본 7일 후 UTC cleanup에서 삭제하고 사용자가 즉시 삭제할 수 있다. object가 이미 없으면 삭제 완료로 정합성을 맞추고, 경로 이탈·I/O 실패는 실패 시각을 기록해 다음 cleanup에서 재시도한다.
9. 로그에서 비밀번호, JWT, API Secret, 개인정보를 마스킹한다.
10. 관리자 API는 IP 제한 또는 별도 관리자 권한을 둔다.
11. API는 개발·베타에서 CSP Report-Only로 위반을 관찰하고 production에서 enforce한다. 정적 프론트 build는 동일한 enforce 헤더 파일을 생성한다.
