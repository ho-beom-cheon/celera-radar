...
MVP Web SaaS는 `users` 테이블을 기준으로 이메일/비밀번호 인증과 JWT 발급을 처리한다. 기존 seed/dev 사용자 호환을 위해 `email`과 `password_hash`는 nullable을 유지하되, 실제 회원가입 사용자는 둘 다 값이 있어야 한다.
...
| `password_hash` | `VARCHAR(255)` | Y |  | BCrypt 등 안전한 방식으로 저장한 비밀번호 해시 |
| `role` | `VARCHAR(30)` | N | `USER` | 인증/관리자 권한. `USER`, `ADMIN` |
...
인증 요청 제한으로 실제 차단이 시작된 시점을 감사 가능한 보안 이벤트로 저장한다. 이메일과 네트워크 주소 원문은 저장하지 않고 애플리케이션에서 SHA-256 hash로 변환한다.

| 컬럼 | 타입 | Nullable | 설명 |
|---|---|---:|---|
| `id` | `BIGINT` | N | 이벤트 PK |
| `event_type` | `VARCHAR(40)` | N | 현재 `RATE_LIMITED` |
| `action` | `VARCHAR(40)` | N | `LOGIN`, `SIGNUP`, `PASSWORD_RESET` |
| `subject_hash` | `VARCHAR(64)` | Y | 정규화 계정 식별값 SHA-256 hex |
| `network_hash` | `VARCHAR(64)` | Y | 서버 관측 원격 주소 SHA-256 hex |
| `retry_after_seconds` | `INT` | Y | 기록 시점의 재시도 대기 시간 |
| `occurred_at` | `TIMESTAMPTZ` | N | 차단 이벤트 발생 시각 |

같은 제한 구간의 반복 차단은 이벤트를 중복 생성하지 않는다. 인덱스는 액션·발생 시각과 계정 hash·발생 시각 기준 조회를 지원한다.
...
## 10.3 `feature_usage_events`

베타/수익화 검증을 위한 사용자 행동 이벤트를 append-only로 저장한다. 실제 결제 연동 전까지는 upgrade CTA, limit hit, 핵심 기능 사용 여부를 기록하는 용도로 사용한다.
...
| `feature_key` | `VARCHAR(80)` | 기능 영역. 예: `keyword`, `candidate`, `store_margin`, `plan` |
| `event_name` | `VARCHAR(120)` | 이벤트명. 예: `keyword.created`, `plan.limit_hit` |
| `plan_code` | `VARCHAR(30)` | 이벤트 시점 요금제 |
| `entity_type` | `VARCHAR(80)` | 관련 엔티티 유형 nullable |
| `entity_id` | `BIGINT` | 관련 엔티티 ID nullable |
...
```text
idx_feature_usage_events_user_time(user_id, occurred_at)
idx_feature_usage_events_name_time(event_name, occurred_at)
idx_feature_usage_events_feature_time(feature_key, occurred_at)
```

## 10.4 `usage_metrics_daily`

사용자별 일 단위 사용량과 수익화 신호를 집계한다. 운영 화면과 베타 검증 리포트는 원천 이벤트가 아니라 이 테이블을 우선 조회한다.
...
| `user_id` | `BIGINT` | FK |
| `plan_code` | `VARCHAR(30)` | 집계 시점 요금제 |
| `active_keyword_count` | `INTEGER` | 활성 키워드 수 |
| `keyword_created_count` | `INTEGER` | 일별 키워드 등록 수 |
| `snapshot_view_count` | `INTEGER` | snapshot 조회 수 |
| `csv_upload_count` | `INTEGER` | CSV/XLSX 업로드 또는 preview 수 |
| `candidate_saved_count` | `INTEGER` | 후보 저장 수 |
| `alert_read_count` | `INTEGER` | 알림 읽음 수 |
| `store_margin_view_count` | `INTEGER` | 내 상품 마진 화면 조회 수 |
| `limit_hit_count` | `INTEGER` | 요금제 제한 도달 수 |
| `created_at` | `TIMESTAMPTZ` | 생성일 |
| `updated_at` | `TIMESTAMPTZ` | 수정일 |
...
```text
uk_usage_metrics_daily_user_date(user_id, metric_date)
ck_usage_metrics_daily_counts_non_negative
```
...
V004__create_trend_snapshots.sql
V005__create_wholesale_tables.sql
V006__add_wholesale_upload_image_mapping.sql
